...
import type { ClientFeedbackComment } from '../types/database'

type CommentWithAuthor = ClientFeedbackComment & { author_name?: string }
...
export default function FeedbackComments({
  feedbackId, clientId, orgId,
}: { feedbackId: string; clientId: string; orgId: string }) {
  const { user } = useAuth()
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setSubmitting(true)
    setError('')
    const { error: insertError } = await supabase.from('client_feedback_comments').insert({
    ...
    setBody('')
    qc.invalidateQueries({ queryKey: ['feedback-comments', feedbackId] })
  }
